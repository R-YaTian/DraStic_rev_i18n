...
<b>启用控制器线性扳机键
使用外接手柄上的线性扳机键，您需要先将线性扳机键映射到DS按键或是特殊功能键。
若您的控制器没有线性扳机或是控制器的L2/R2会同时触发按键和线性扳机事件(此时映射的按键可能会触发两次)，建议您关闭该选项

<b>控制器右摇杆模式
用您外接手柄上的右摇杆（或者触控板）来进行某些游戏的触屏操作
对于FPS游戏来说，这很有用。可以依次尝试四种不同模式（Dpad、FPS模式、摇杆模式或是触笔模式），看看哪种更适合您的游戏
如果设为“触笔模式”，则摇杆将直接移动触笔指针，该指针可通过指定按钮按下

<b>控制器摇杆旋转模式
决定控制器左右摇杆的旋转模式，支持左转、右转与旋转180度三种旋转方式
...
<b>单屏布局输出DS下屏幕
在单屏布局中，默认输出DS的下屏幕而非上屏幕。
如果您的双屏手持游戏设备以下屏幕为主屏幕，强烈建议您开启此选项以便于获得更好的上下屏切换体验。

<b>快捷菜单使用TV模式风格
启用后，快捷菜单将使用TV模式风格，允许您选择屏幕布局与切换滤镜配置
(滤镜配置将针对不同游戏单独保存，而非直接覆盖全局设置)
...
<b>缓存压缩的游戏到临时文件夹
启用此项，压缩的游戏会自动解压并缓存在您的内部存储中。这样会延长首次加载时间，但可以节省内存。
...
当您使用外接手柄时，使用左摇杆或者方向键进行选择，使用映射为A键的按键决定。映射为B键的按键用于取消。
例如，如果您想通过快捷菜单读取快速存档，按下十字键左键，然后按A键。
如果您想加速，则只需在打开该菜单之后按下A键
